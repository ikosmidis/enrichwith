...
vignette: >
  %\VignetteIndexEntry{exponential family of distributions}
...
`family` objects with a function that links $\theta$ with $\mu$, the
function $b(\theta)$, the first two derivatives of $V(\mu)$,
$a(\zeta)$ and its first four derivatives, and $c_1(y)$ and
$c_2(y)$. To illustrate, let's write a function that reconstructs the
densitities and probability mass functions from the components that result from enrichment
```{r, echo = TRUE, eval = TRUE}
