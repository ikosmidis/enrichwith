...
package, which estimates binomial-response GLMs using iterative ML fits
...
Consider a parametric statistical model $\mathcal{P}_\theta$ with
unknown parameter $\theta \in \Re^p$ and the iteration $$\theta^{(k +
1)} := \theta^{(k)} + \left\{i(\theta^{(k)})\right\}^{-1}
s(\theta^{(k)}) - c(\theta^{(k)}) b(\theta^{(k)})$$ where
$\theta^{(k)}$ is the value of $\theta$ at the $k$th iteration,
$s(\theta)$ is the gradient of the
...
[bias](https://en.wikipedia.org/wiki/Bias_of_an_estimator) of the [ML
