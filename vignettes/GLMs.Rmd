...
**enrichwith** also provides the `get_dmodel_function`, `get_pmodel_function` and `get_qmodel_function` methods, which can be used to evaluate densities or probability mass functions, distribution functions, and quantile functions, respectively, under the fitted design and at arbitrary parameter values.
...
```
or at a different response under the same fitted design
```{r, echo = TRUE, eval = TRUE}
...
cML_qmodel(probs, coefficients = c(-0.01, 0.02, -0.01, 0),
           dispersion = 0.1)
```
returns `new_response` (up to numerical tolerance).
